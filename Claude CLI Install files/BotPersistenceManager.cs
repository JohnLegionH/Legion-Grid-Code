...
";
        ...
        public bool IsEnabled()
        {
            ...
            // Check region-level toggle (stored in RegionSettings)
            // RegionSettings.AllowBotPersistence — defaults to true if field exists
            // If the field hasn't been added yet, default to true
            try
            {
                return m_scene.RegionInfo.RegionSettings.AllowBotPersistence;
            }
            catch
            {
                // Field may not exist yet — default to enabled if grid is enabled
                return true;
            }
        }
