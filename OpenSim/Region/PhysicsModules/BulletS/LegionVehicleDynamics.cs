...
{
    /// <summary>
    /// Halcyon-derived vehicle dynamics engine for BulletSim.
    /// Replaces BSDynamics with production-tested vehicle physics from InWorldz.
    /// </summary>
    public sealed class LegionVehicleDynamics : BSActor
    {
        private static readonly ILog m_log
            = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
        private static readonly string LogHeader = "[LEGION VEHICLE]";

        // The prim this controller belongs to
        ...
        // Constructor
        // =====================================================================
        public LegionVehicleDynamics(BSScene myScene, BSPrim myPrim, string actorName)
            : base(myScene, myPrim, actorName)
        {
            ...
            VDetailLog("{0},LegionVehicleDynamics.Creation", ControllingPrim.LocalID);
        }
        ...
        {
            get { return (_props.Type != LegionVehicleType.None && ControllingPrim.IsPhysicallyActive); }
        }
        ...
        #region Vehicle Parameter Setting — routes from OpenSim Vehicle enum to internal enums
        ...
        public void ProcessTypeChange(Vehicle pType)
        {
            ...
            if (newType == LegionVehicleType.None)
            ...
            VDetailLog("{0},LegionVehicle.ProcessTypeChange,type={1}", ControllingPrim.LocalID, newType);
        }
        ...
        {
            VDetailLog("{0},LegionVehicle.ProcessFloat,param={1},val={2}", ControllingPrim.LocalID, pParam, pValue);
        ...
        {
            VDetailLog("{0},LegionVehicle.ProcessVector,param={1},val={2}", ControllingPrim.LocalID, pParam, pValue);
        ...
        {
            VDetailLog("{0},LegionVehicle.ProcessRotation,param={1},val={2}", ControllingPrim.LocalID, pParam, pValue);
        ...
        public void ProcessVehicleFlags(int pParam, bool remove)
        {
            ...
            VDetailLog("{0},LegionVehicle.ProcessFlags,param={1},remove={2},result={3}",
                ControllingPrim.LocalID, pParam, remove, _props.Flags);
        }

        #endregion // Vehicle Parameter Setting

        #region BSActor Overrides

        public override void Refresh()
        {
            m_physicsScene.PostTaintObject("LegionVehicle.Refresh", ControllingPrim.LocalID, delegate()
            {
        ...
        {
            VDetailLog("{0},LegionVehicle.Dispose", ControllingPrim.LocalID);
            UnregisterForSceneEvents();
            _props.Type = LegionVehicleType.None;
            Enabled = false;
        }

        #endregion // BSActor Overrides
        ...
        private void SetPhysicalParameters()
        {
            if (IsActive)
            {
                ...
                // Vehicle controls its own friction
                ...
                // Vehicle applies gravity manually — set physics gravity to zero
                ControllingPrim.Linkset.SetPhysicalGravity(Vector3.Zero);

                // Prevent BulletSim from sleeping the vehicle body
                m_physicsScene.PE.ForceActivationState(ControllingPrim.PhysBody, ActivationState.DISABLE_DEACTIVATION);

                VDetailLog("{0},LegionVehicle.SetPhysicalParameters,mass={1},gravity=manual",
                    ControllingPrim.LocalID, m_vehicleMass);
            }
            else
            {
        ...
        /// <summary>
        /// Called every BulletSim physics frame via BSScene.BeforeStep event.
        ...
        internal void Step(float pTimestep)
        {
            ...
            _rotation = ControllingPrim.ForceOrientation * _vframe;

            // Velocity anti-jitter: per-axis, when velocities are opposing, average to eliminate spike
            Vector3 physAngVel = ControllingPrim.ForceRotationalVelocity;
            ...
            Vector3 physLinVel = ControllingPrim.ForceVelocity;
            ...
            float groundHeight = GetTerrainHeight(ControllingPrim.ForcePosition);
            if (ControllingPrim.ForcePosition.Z - groundHeight < LegionVehicleLimits.MaxGroundPenetration)
            {
            ...
            // -------------------------------------------------------
            // Angular deflection — turning toward direction of movement
            ...
            // -------------------------------------------------------
            // Linear deflection — changing velocity toward forward axis
            ...
            // -------------------------------------------------------
            // Sled movement — gravity-assisted force on slopes
            ...
            _props.Dynamics.LastPosition = ControllingPrim.ForcePosition;
            _props.Dynamics.Timestep = timeStep;
            _props.Dynamics.LocalLinearVelocity = _localLinearVel;
            _props.Dynamics.LocalAngularVelocity = _localAngularVel;

                        VDetailLog("{0},LegionVehicle.Step,done,pos={1},vel={2},angvel={3}",
                ControllingPrim.LocalID, ControllingPrim.ForcePosition,
                ControllingPrim.ForceVelocity, ControllingPrim.ForceRotationalVelocity);
        }

        internal void PostStep(float pTimestep)
        {
            // Reserved for post-step operations
        }
        ...
        private void MoveLinear(Vector3 direction)
        {
            ...
            ControllingPrim.ActivateIfPhysical(false);

            VDetailLog("{0},LegionVehicle.MoveLinear,dir={1}", ControllingPrim.LocalID, direction);
        }
        ...
        private void MoveAngular(Vector3 direction)
        {
            ...
            VDetailLog("{0},LegionVehicle.MoveAngular,dir={1}", ControllingPrim.LocalID, direction);
        }
        ...
        {
            Vector3 currpos = ControllingPrim.ForcePosition;
        ...
        private void ApplyGravity(float timeStep)
        {
            ...
            Vector3 gravity = ControllingPrim.ComputeGravity(buoyancy);
            ...
            // Apply as a force
            ControllingPrim.AddForce(true, appliedGravity);

            VDetailLog("{0},LegionVehicle.ApplyGravity,buoy={1},grav={2},force={3}",
                ControllingPrim.LocalID, buoyancy, gravity, appliedGravity);
        }
        ...
        private void TorqueFini()
        {
            ...
            {
                ControllingPrim.ForceRotationalVelocity = ControllingPrim.ForceRotationalVelocity + _accumTorqueVelChange;
                changed = true;
            }
            ...
            {
                ControllingPrim.AddAngularForce(true, _accumTorqueImpulse);
                changed = true;
            }
        ...
        /// Applies linear motor offset if set.
        /// </summary>
        private void ApplyLinearVelocityChange(Vector3 deltaV)
        {
            Vector3 loffset = _props.GetVec(VehVectorParam.LinearMotorOffset);

            if (loffset == Vector3.Zero)
            {
                ControllingPrim.ForceVelocity = ControllingPrim.ForceVelocity + deltaV;
                m_physicsScene.PE.PushUpdate(ControllingPrim.PhysBody);
            }
            else
            {
                // VelocityChange is not directly supported with offset, simulate via impulse
                Vector3 force = deltaV * m_vehicleMass;
                ControllingPrim.AddForceImpulse(force, false, true);
            }
        }

        /// <summary>
        /// Apply a mass-scaled linear impulse.
        /// Equivalent to Halcyon AddForce with ForceMode.Impulse.
        /// </summary>
        private void ApplyLinearImpulse(Vector3 impulse)
        {
            ControllingPrim.AddForceImpulse(impulse, false, true);
        }

        /// <summary>
        /// Apply a continuous force (mass-dependent, accumulated over time).
        /// Equivalent to Halcyon AddForce with ForceMode.Force.
        /// </summary>
        private void ApplyLinearForce(Vector3 force)
        {
            ControllingPrim.AddForce(true, force);
        }

        private float GetTerrainHeight(Vector3 pos)
        {
            return m_physicsScene.TerrainManager.GetTerrainHeightAtXYZ(pos);
        }

        private float GetWaterLevel(Vector3 pos)
        {
            return m_physicsScene.TerrainManager.GetWaterLevelAtXYZ(pos);
        }
        ...
        private void MitigateVelocitySpiking(float actualStep)
        {
            ...
            {
                Vector3 remvel = ControllingPrim.ForceRotationalVelocity * Quaternion.Inverse(_rotation);
                remvel.Z = 0;
                remvel *= _rotation;
            ...
            if (accel > 50.0f)
            {
                Vector3 remvel = ControllingPrim.ForceVelocity * Quaternion.Inverse(_rotation);
                remvel.X = 0;
                remvel.Y *= 0.5f;
                remvel *= _rotation;
        ...
        #region Simulation Methods — Angular & Linear Deflection

        /// <summary>
        /// Rotates vehicle toward direction of movement.
        /// Ported from Halcyon VehicleDynamics.SimulateAngularDeflection().
        /// </summary>
        ...
        /// Ported from Halcyon VehicleDynamics.SimulateLinearDeflection().
        /// </summary>
        ...
        #region Sled Movement

        /// <summary>
        /// Gravity-assisted force on slopes for sled vehicle type.
        /// Ported from Halcyon VehicleDynamics.SimulateSledMovement().
        /// </summary>
        private void SimulateSledMovement(float timeStep)
        {
            ...
            {
                force = new Vector3(-BSParam.Gravity * 3.0f, 0f, 0f);
        ...
        private void SimulateHover(float timeStep)
        {
            ...
            Vector3 pos = ControllingPrim.ForcePosition;
            ...
            {
                targetBase = GetWaterLevel(pos);
            }
            else if ((_props.Flags & LegionVehicleFlags.HoverTerrainOnly) != 0)
            {
                targetBase = GetTerrainHeight(pos);
            }
            ...
            else
            {
                ...
                targetBase = Math.Max(GetTerrainHeight(pos), GetWaterLevel(pos));
            }
            ...
            VDetailLog("{0},LegionVehicle.SimulateHover,targetBase={1},targetZ={2},currentZ={3},error={4},corrVel={5}",
                ControllingPrim.LocalID, targetBase, targetZ, currentZ, error, correctionVel);
        }
        ...
        private void SimulateVerticalAttractor(float timeStep, uint frameNum, out Vector3 attractionForces, out float angle, out bool inverted)
        {
            ...
            if (timescale < LegionVehicleLimits.MaxAttractTimescale)
            {
                    ...
                    (frameNum - _props.Dynamics.LastVerticalFrameNumber) < (LegionVehicleLimits.MaxAttractDormancy / timeStep))
                {
                    ...
                    Vector3 remvel = Vector3.Zero;
                    Vector3 wtensor = ControllingPrim.Inertia;
        ...
        /// <summary>
        /// Converts roll to yaw rotation.
        /// Ported from Halcyon VehicleDynamics.SimulateBankingToYaw().
        /// </summary>
        ...
        private void SimulateMotors(float timeStep, uint frameNum, Vector3 aforces)
        {
            ...
            else
            {
                    ...
                    Math.Abs(worldvel.Z) >= LegionVehicleLimits.ThresholdLinearMotorDeltaV)
                {
                    ...
                    if (worldvel.Z > 0)
                    {
                        worldvel.Z += BSParam.Gravity * (1.0f - buoy);
                        if (worldvel.Z < 0) worldvel.Z = 0;
                    }
        ...
        // =================================================================
        // Public accessor for vehicle type (used by BSPrim.VehicleType getter)
        // =================================================================
        public Vehicle Type
        {
            get
