...
namespace InWorldz.Phlox.Glue
{
	public interface ISystemAPI
	{
        ...
        string iwRezAt(string inventory, int rezAtRoot, Vector3 pos, Vector3 vel, Quaternion rot, int param);
        ...
        string llDetectedRezzer(int number);
        string llGetInventoryDesc(string item);
        string llGetInventoryAcquireTime(string item);
        string llName2Key(string name);
        float llGetRegionTimeOfDay();
        float llGetSimStats(int statType);
        int llGetSPMaxMemory();
        LSLList llGetObjectAnimationNames();
        ...
        LSLList llGetAttachedListFiltered(string avatar, int attachmentPoint);
        int llIsFriend(string agent);
        LSLList llGetVisualParams(string agent, LSLList paramIndices);
        string llGetNotecardLineSync(string name, int line);
        int llFindNotecardTextSync(string name, string text, int line, int caseSensitive);
        ...
        LSLList llSortListStrided(LSLList src, int stride, int stride_index, int ascending);
        int llListFindListNext(LSLList src, LSLList test, int ofs);
        ...
        int llLinksetDataWriteProtected(string name, string value, string pass);
        int llLinksetDataDelete(string name);
        int llLinksetDataDeleteProtected(string name, string pass);
        void llLinksetDataReset();
        LSLList llLinksetDataDeleteFound(string pattern, string pass);
        int llLinksetDataCountFound(string pattern);
        LSLList llLinksetDataListKeys(int start, int count);
        LSLList llLinksetDataFindKeys(string pattern, int start, int count);

        // ── Tier 1: EEP / Environment ──
        int llGetDayLength();
        int llGetDayOffset();
        int llGetRegionDayLength();
        int llGetRegionDayOffset();
        Vector3 llGetMoonDirection();
        Quaternion llGetMoonRotation();
        Quaternion llGetSunRotation();
        Vector3 llGetRegionSunDirection();
        Quaternion llGetRegionSunRotation();
        Vector3 llGetRegionMoonDirection();
        Quaternion llGetRegionMoonRotation();
		Vector3 llGetRegionLightDir();

        // ── Tier 2: PBR / GLTF Materials ──
        string llGetRenderMaterial(int face);
        void llSetRenderMaterial(string materialId, int face);
        int llIsLinkGLTFMaterial(int link, int face);
        int llSetLinkGLTFOverrides(int link, int face, LSLList overrides);
        void llRenderMaterial(string materialId);
        ...
        LSLList llGetClosestNavPoint(Vector3 point, LSLList options);
        LSLList llGetStaticPath(Vector3 start, Vector3 end, float radius, LSLList parameters);
        void llExecCharacterCmd(int command, LSLList parameters);
        void llDeleteCharacter();
		// -- Pathfinding Character Functions --
		void llCreateCharacter(LSLList options);
		void llNavigateTo(Vector3 pos, LSLList options);
		void llUpdateCharacter(LSLList options);
		void llPursue(string targetID, LSLList options);
		void llEvade(string targetID, LSLList options);
		void llFleeFrom(Vector3 source, float distance, LSLList options);
        ...
        int llVerifyRSA(string data, string signature, string publicKeyPem, string algorithm);

        // ── Tier 7: EEP Environment ──
        LSLList llGetEnvironment(Vector3 pos, LSLList paramList);
        int llSetEnvironment(Vector3 pos, LSLList paramList);
        int llReplaceEnvironment(Vector3 pos, string environment, int track, int day_length, int day_offset);

        // ── Tier 7b: Agent Environment + User Key ──
        string llRequestUserKey(string username);
        int llSetAgentEnvironment(string agentID, float transition, LSLList paramList);
        int llReplaceAgentEnvironment(string agentID, float transition, string environment);

        // ── Tier 8: Remaining SL Gaps ──
        string llGetExperienceErrorMessage(int error);
        void llSetAgentRot(Quaternion rot, int relative);
        ...
        int llHash(string src);
        void llLinkAdjustSoundVolume(int link, float volume);
        void llLinkSetSoundQueueing(int link, int queue);
        void llLinkSetSoundRadius(int link, float radius);
        void llLinkStopSound(int link);
        void llScriptProfiler(int flags);
        void llGiveAgentInventory(string destination, string inventory);
        void llMapBeacon(string agent, string text, Vector3 color);
        ...
        void llRequestExperiencePermissions(string agent, string name);
        int llAgentInExperience(string agent);
        LSLList llGetExperienceDetails(string experienceId);
