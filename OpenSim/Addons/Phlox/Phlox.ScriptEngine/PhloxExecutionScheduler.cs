...
namespace Phlox.ScriptEngine
{
    internal class PhloxExecutionScheduler
    {
        ...
        // Async call thread pool (for long-running syscalls like HTTP, dataserver, etc.)
        private readonly SmartThreadPool m_AsyncPool;
        ...
        public PhloxExecutionScheduler(WorkArrivedDelegate workArrived, PhloxEngine engine, IWorldComm worldComm)
        {
            ...
            m_AsyncPool = new SmartThreadPool(new STPStartInfo
            {
                MinWorkerThreads = 0,
                MaxWorkerThreads = 6,
                IdleTimeout = 60 * 1000
            });
            m_AsyncPool.Name = "Phlox Async";
        }
        ...
        private void CheckSyscallTimeouts()
        {
            ...
            foreach (var e in stale)
                m_log.WarnFormat(
                    "[PhloxExe]: Script {0} (asset {1}) has been parked in Syscall for {2}ms with no SysReturn — likely a wedged async syscall (an API body that failed to resume). Run 'phlox scripts syscall' to inspect.",
                    e.item, e.asset, e.ms);
        }
